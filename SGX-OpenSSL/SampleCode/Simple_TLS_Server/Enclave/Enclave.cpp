...
//#define CADIR NULL
//#define CERTFILE "www.example.com.cert.pem"
...
static SSL_CTX *create_context()
{
    ...
    method = SSLv23_server_method(); //TLSv1_server_method();

    ctx = SSL_CTX_new(method);
    
    //SSL_CTX_use_certificate_file(ctx, CERTFILE, SSL_FILETYPE_PEM);
    //if (SSL_CTX_load_verify_locations(ctx, CAFILE, CADIR) != 1)
    //    printe("Error loading CA file and/or directory");
    //if (SSL_CTX_set_default_verify_paths(ctx) != 1)
    //    printe("Error loading default CA file and/or directory");
