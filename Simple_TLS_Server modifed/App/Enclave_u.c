...
static sgx_status_t SGX_CDECL Enclave_ocall_readCertFile(void* pms)
{
	ms_ocall_readCertFile_t* ms = SGX_CAST(ms_ocall_readCertFile_t*, pms);
	ms->ms_retval = ocall_readCertFile((const char*)ms->ms_file);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_readKeyFile(void* pms)
{
	ms_ocall_readKeyFile_t* ms = SGX_CAST(ms_ocall_readKeyFile_t*, pms);
	ms->ms_retval = ocall_readKeyFile((const char*)ms->ms_file);
...
static const struct {
	...
	25,
	{
		...
		(void*)Enclave_ocall_readCertFile,
		(void*)Enclave_ocall_readKeyFile,
