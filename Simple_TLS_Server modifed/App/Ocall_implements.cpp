...
char *ocall_readCertFile(const char *file)
{
    FILE *fp;
    char *cert = (char *)calloc(256,sizeof(char *));
    char line[255];
    fp = fopen(file,"r");
    if(fp==NULL){
    ...
    while(fgets(line, 255, fp)!=NULL){
        sprintf(cert,"%s%s",cert,line);
    }
    int str = strlen(cert)-1;
    if(cert[str] == '\n'){
        cert[str]='\0';
    }    
...
char *ocall_readKeyFile(const char *file)
{
    FILE *fp;
